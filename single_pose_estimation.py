...
def run(estimation_model: str, tracker_type: str, classification_model: str,
        label_file: str, camera_id: int, width: int, height: int, ip: str) -> None:
  ...
  while cap.isOpened():
    ...
    image = cv2.resize(image, dsize=None, fx=2.5, fy=2.5)
    ...
    # print(list_persons[0].keypoints[0].coordinate.x) nose x
    person = list_persons[0]
    nose_y = person.keypoints[0].coordinate.y
    l_shoulder = person.keypoints[5].coordinate.y
    r_shoulder = person.keypoints[6].coordinate.y
    l_elbow = person.keypoints[7].coordinate.y
    r_elbow = person.keypoints[8].coordinate.y
    l_wrist = person.keypoints[9].coordinate.y
    r_wrist = person.keypoints[10].coordinate.y
    if l_wrist < l_elbow < l_shoulder and r_wrist < r_elbow < r_shoulder:
        if l_wrist > nose_y and r_wrist > nose_y:
            lis.append(True)
            if len(lis) > 10:
                sendudp(ip)
                lis = []
    ...
    # Show the FPS and info
    fps_text = 'FPS = ' + str(int(fps))
    text_location = (left_margin, row_size)
    cv2.putText(image, fps_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)
    nose_text = 'NOSE: ' + str(person.keypoints[0].coordinate)
    l_shoulder_text = 'LEFT SHOULDER: ' + str(person.keypoints[5].coordinate)
    r_shoulder_text = 'RIGHT SHOULDER: ' + str(person.keypoints[6].coordinate)
    l_elbow_text = 'LEFT ELBOW: ' + str(person.keypoints[7].coordinate)
    r_elbow_text = 'RIGHT ELBOW: ' + str(person.keypoints[8].coordinate)
    l_wrist_text = 'LEFT WRIST: ' + str(person.keypoints[9].coordinate)
    r_wrist_text = 'RIGHT WRIST: ' + str(person.keypoints[10].coordinate)
    cv2.putText(image, nose_text, (left_margin, 2*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, l_shoulder_text, (left_margin, 3*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, r_shoulder_text, (left_margin, 4*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, l_elbow_text, (left_margin, 5*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, r_elbow_text, (left_margin, 6*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, l_wrist_text, (left_margin, 7*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness)
    cv2.putText(image, r_wrist_text, (left_margin, 8*row_size), cv2.FONT_HERSHEY_PLAIN, 
            font_size,text_color, font_thickness) 
...
def sendudp(ip):
    ...
    udp_socket.sendto("0".encode('utf-8'), (ip, 32000))
    udp_socket.close()
...
def main():
  ...
  parser.add_argument(
          '--ip', 
          help='The ip who will be control.',
          required=True)
  args = parser.parse_args()

  run(args.model, args.tracker, args.classifier, args.label_file,
      int(args.cameraId), args.frameWidth, args.frameHeight, args.ip)
